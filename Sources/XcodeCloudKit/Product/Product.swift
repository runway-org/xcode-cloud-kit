public struct Product {
    ...
    public func workflows() async throws -> [Workflow] {
        ...
        return allWorkflows.data.map { Workflow(id: $0.id, name: $0.attributes.name, product: self, transport: transport) }
    }
    ...
    public func workflow(withId id: String) async throws -> Workflow {
        ...
        return Workflow(id: workflow.data.id, name: workflow.data.attributes?.name ?? "", product: self, transport: transport)
    }
    ...
    public func workflow(withName name: String) async throws -> Workflow? {
        try await transport.perform(request: RequestBuilder.allWorkflows(for: id))
            .data
            .map { Workflow(id: $0.id, name: $0.attributes.name, product: self, transport: transport) }
